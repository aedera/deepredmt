...
def fit(fin,
        ...
        # # ### FINE tuning
        # # # restore best model found during pre-training
        # # ##### <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        # # save_model = False
        # datetime_tag = "210420-1541"
        # # ###################

        # model_file = "./models/" + 'deepredmt/' + datetime_tag + ".tf"
        # pretrained = tf.keras.models.load_model(model_file, compile=False)
        # model = tf.keras.Model(inputs=pretrained.input, outputs=pretrained.output)
        # model.summary()
